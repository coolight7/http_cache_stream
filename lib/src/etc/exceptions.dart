...
class InvalidRangeRequestException extends InvalidCacheException {
  ...
  ) : super(uri, 'Invalid Download Range Response | Request: $request | Response: $response');
}
...
class InvalidCacheLengthException extends InvalidCacheException {
  InvalidCacheLengthException(Uri uri, int length, int expected)
      : super(uri, 'Invalid cache length | Length: $length, expected $expected (Diff: ${expected - length})');
}
...
class InvalidHttpStatusCode extends HttpException {
  InvalidHttpStatusCode(Uri uri, int expected, int result) : super('Invalid HTTP status code | Expected: $expected | Result: $result', uri: uri);
}
