abstract class HttpRange {
  ...
  const HttpRange(this.start, this.end, {this.sourceLength})
      : assert(start >= 0, 'start must be null or non-negative'),
        assert(end == null || (end >= start), 'end must be null or greater than or equal to start');
  ...
  static (int? start, int? end, int? sourceLength) parse(String rangeHeader) {
    ...
    if (rangeHeader.startsWith('bytes')) {
      rangeHeader = rangeHeader.replaceAll(RegExp(r'[^0-9/-]'), ''); // Clean the header to just numbers and separators
      if (rangeHeader.isNotEmpty) {
        ...
        if (rangeStr.isNotEmpty) {
          if (rangeStr.startsWith('-')) {
            ...
            if (suffixLength != null && suffixLength >= 0 && sourceLength != null && sourceLength > suffixLength) {
            ...
            if (start != null) {
              ...
              if (end != null && (end < start || (sourceLength != null && end >= sourceLength))) {
  ...
  bool operator ==(Object other) {
    ...
    return other is HttpRange && start == other.start && end == other.end && sourceLength == other.sourceLength;
  }
...
int? _parseIntFromList(List<String> list, int index) {
  ...
  return result != null && result.isFinite ? result : null; // Ensure the number is finite
}
